#!/usr/bin/env python3
"""Minimal test: render a PDF page using textual-image SixelWidget.

Run in WezTerm:
    AURA_RENDERER=sixel .venv/bin/python scripts/test_sixel_render.py [path/to/pdf]

If no PDF is given, renders a synthetic test image.
"""
...
class TestApp(App):
    ...
    def compose(self) -> ComposeResult:
        yield Header()
        yield SixelImage(self._img, id="image")
        yield Static("Sixel test — press q to quit", id="status")
        yield Footer()
